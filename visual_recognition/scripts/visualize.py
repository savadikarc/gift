...
from wegeft.vis_utils import ClusterVisualizer
...
group.add_argument(
    ...
    type=int,
    default=-1,
    help="WeGeFT hypernet layer to visualize if using any block other than simple_block",
)
group.add_argument(
    "--epoch_to_visualize",
    type=int,
...
def main():
    ...
    log_path = os.path.join(output_dir, f"visualization.log")
    ...
    visualizer = ClusterVisualizer(model, device=device, wegeft_layer=args.wegeft_layer, mean=data_config["mean"], std=data_config["std"])
    args.visualization_dir = Path(output_dir, f"visualizations_classwise")
    args.visualization_dir.mkdir(exist_ok=True, parents=True)
...
def validate(
    ...
    with torch.no_grad():
        for batch_idx, data in enumerate(loader):
            ...
            target_cls = target[0].item()
            if target_cls != args.class_to_visualize:
                continue
            ...
            is_valid_cls = target_cls == args.class_to_visualize
            is_counter_full = visualizer.num_visualizations >= args.num_visualizations
            register_hooks = is_valid_cls and not is_counter_full
            if register_hooks:
            ...
            # Visualize
            output_cls = output[0].argmax().item()
            if output_cls == target_cls and register_hooks:
                visualizer.save_visualizations(args.visualization_dir, target_cls)
                vis_counter[target_cls] += 1
            elif register_hooks:
                visualizer.reset()
