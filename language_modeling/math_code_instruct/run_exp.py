import inspect
import warnings
from peft import get_peft_model, LoraConfig, AdaLoraConfig, TaskType, WeGeFTConfig
import os
import hydra
...
def run_exp(cfg: DictConfig):
    ...
    if cfg.get("gradient_checkpointing", False):
        ...
        model = prepare_model_for_peft(model, use_gradient_checkpointing=cfg.get("gradient_checkpointing", False), gradient_checkpointing_kwargs={"use_reentrant": False})
    additional_kwargs = {}
    ...
    if use_peft and cfg.peft.get("dora", False):
        ...
        num_non_trainable = sum([p.numel() for p in model.parameters() if not p.requires_grad])
        percent_trainable = n_params / num_non_trainable * 100
        print(f"*****Trainable params: {percent_trainable}")
        rate = {
    ...
    elif use_peft and cfg.peft.get("wegeft", False):
        target_modules = eval(cfg.peft.lora_target_modules)
        tied_modules = eval(cfg.peft.tied_modules)
        transform_dim = eval(cfg.peft.transform_dim)
        peft_config = WeGeFTConfig(
            target_modules=target_modules,
            tied_modules=tied_modules,
            transform_dim=transform_dim,
            r=cfg.peft.lora_r,
            wegeft_alpha=cfg.peft.lora_alpha,
            init_wegeft_weights=cfg.peft.init_wegeft_weights,
            task_type="CAUSAL_LM",
        )
        model = get_peft_model(model, peft_config)
        n_params = sum([p.numel() for p in model.parameters() if p.requires_grad])
    ...
    report_to = ["none"] if not cfg.wandb.use_wandb else ["wandb"]
